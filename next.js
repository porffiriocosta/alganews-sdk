...
async function generateNewDir (directoryName) {
    const dirPath = path.resolve(__dirname, '..', 'erjs-git', directoryName)
...
async function main () {
    ...
    if (!chapter)
        throw new Error('Está faltando o módulo')

    if (!number)
        throw new Error('Está faltando a aula')

    if (!title)
        throw new Error('Está faltando o título da aula')
    ...
    try {
        ...
        const projectHasBeenCopied = await copyProjectToExistingDirectory(newDirectoryFullPath)
